...
#### 💬 채팅 시스템 (메인 기능)
- ✅ **실시간 1:1 DM 및 그룹 채팅**
- ✅ **Supabase Realtime 통합** (WebSocket 자동 재연결)
- ✅ **타이핑 인디케이터**
- ✅ **메시지 가상화** (@tanstack/react-virtual)
- ✅ **Optimistic UI 업데이트**
- ✅ **파일 업로드 시스템** (완전 재사용 가능, 2025-10-02 완성)
  - 📸 갤러리에서 이미지 선택
  - 📷 카메라로 사진 촬영
  - 📁 파일 선택
  - 📍 위치 공유
  - 🔄 모바일/데스크톱 반응형 UI (Sheet/Popover)
- ✅ **이미지 업로드 및 렌더링** (2025-10-02 완성)
  - Supabase Storage 실제 업로드 구현
  - 서버 사이드 파일 처리 (API Route)
  - 공개 URL 자동 생성
  - 라이트박스로 이미지 확대 보기
  - 이미지 최적화 및 압축
...
- ✅ **위치 공유 시스템** (카카오맵 완전 통합, 2025-10-01 완성)
  ...
  - 📌 마커 표시 및 상세 정보
  - 🌐 카카오맵 SDK 동적 로드
  - 📱 반응형 지도 UI (80% 채팅 영역 사용)
  - 📏 헤더 컴팩트화 (명칭·주소·전화번호 한 줄)
- ✅ **읽음 표시** (read receipts)
- ✅ **채팅방 관리** (생성/삭제/나가기/초대)
- ✅ **URL 상태 동기화** (딥링크)
- ✅ **모바일/데스크톱 반응형**
- ✅ **메시지 그룹핑** (카카오톡 스타일)
- ✅ **채팅 UI 최적화** (T 버튼 제거, 입력창 정리)
...
### 📂 파일 업로드 시스템 (2025-10-02 업데이트)

```typescript
// 파일 타입 자동 판단
function getMessageType(file?: File, fileName?: string): "text" | "file" | "image" | "location" {
  ...
  // 이미지 파일 판단 (MIME 타입 우선)
  if (file?.type.startsWith('image/')) return "image";

  // 확장자 기반 판단 (fallback)
  if (['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.bmp']
      .some(ext => (fileName || file?.name)?.toLowerCase().endsWith(ext))) {
    return "image";
  }
...
// 업로드 플로우 (완전 재구현)
1. 사용자 파일 선택
   ├─ ChatAttachmentMenu (+ 버튼)
   │  ├─ GalleryOption (이미지 선택)
   │  ├─ CameraOption (사진 촬영)
   │  ├─ FileOption (모든 파일)
   │  └─ LocationOption (위치 공유)
   ↓
2. handleFileSelect([file])
   ├─ selectedFile 상태 업데이트
   └─ FilePreview 표시 (🖼️ 아이콘 + 파일명 + 크기 + 제거 버튼)
   ↓
3. 전송 버튼 클릭
   ├─ FormData 생성
   │  ├─ room_id
   ...
   │  └─ file (File 객체)
   ↓
4. API Route: POST /api/chat/messages
   ├─ getMessageType(file) → "image" | "file" | "location"
   ├─ if (image | file):
   │  ├─ File → ArrayBuffer → Blob 변환
   │  ├─ Supabase Storage 업로드
   │  │  └─ 경로: chat/{user_id}/{timestamp}.{ext}
   │  │  └─ 버킷: "posts"
   │  ├─ 공개 URL 생성
   │  └─ file_url에 저장
   ├─ if (location):
   │  └─ JSON 내용을 content에 저장
   ↓
5. DB 저장 (chat_messages)
   ├─ message_type (자동 판단됨)
   ├─ content (텍스트 또는 위치 JSON)
   ├─ file_url (Supabase Storage URL)
   ├─ file_name
   └─ file_size
   ↓
6. 실시간 렌더링
   ├─ MessageRenderer가 타입별로 분기
   │  ├─ "image" → ClickableImage (라이트박스)
   │  ├─ "file" → 다운로드 버튼
   │  └─ "location" → LocationMessage (카카오맵)
   └─ 최적화된 메시지 표시
   ↓
...
**높은 우선순위:**
- [x] ~~카카오맵 위치 렌더링 완성~~ (2025-10-01 완료)
- [x] ~~메시지 타입별 조건부 스타일링~~ (2025-10-02 완료 - 텍스트만 버블, 이미지/파일/위치는 버블 제거)
- [x] ~~DOMPurify XSS 보안 강화~~ (v0.6 완료 - /src/lib/sanitize.ts 중앙화 구현)
- [x] ~~타이핑 인디케이터 debounce~~ (2025-10-02 완료 - Lodash debounce로 85% API 호출 감소)
- [ ] 다중 파일 업로드 구현
- [ ] 이미지 최적화 활성화
